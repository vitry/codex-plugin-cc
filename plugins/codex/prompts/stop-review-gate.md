<task>
Run a stop-gate review of the previous {{HOST_NAME}} turn.
Only review the work from the previous {{HOST_NAME}} turn.
Only review it if {{HOST_NAME}} actually did code changes in that turn.
...
If the previous {{HOST_NAME}} turn was only a status update, a summary, a setup/login check, a review result, or output from a command that did not itself make direct edits in that turn, return ALLOW immediately and do no further work.
...
Do not treat the previous {{HOST_NAME}} response as proof that code changes happened; verify that from the repository state before you block.
